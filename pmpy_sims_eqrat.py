...
# Start in reference position offset
if "o" in str(sys.argv[5]):
    startvec = np.array([.2, vt*.7, np.cos(ang)*1e-4,np.sin(ang)*1e-5])
else:
# Start in reference position
    startvec = np.array([.2, vt*.5, np.cos(ang)*1e-4,np.sin(ang)*1e-5])
...
# For lambda of 1
if "o" in str(sys.argv[5]) and "p" in str(sys.argv[5]):
    np.save("pmpy_eqratoff_lamp{}_ang{}_tmax{}_dt{}".format(str(lam).split('.')[-1],str(int(sys.argv[2])),str(int(sys.argv[3])),str(dt).split('.')[-1]),gs3simdatalist)
elif "o" in str(sys.argv[5]):
    np.save("pmpy_eqratoff_lam{}_ang{}_tmax{}_dt{}".format(str(int(sys.argv[1])),str(int(sys.argv[2])),str(int(sys.argv[3])),str(dt).split('.')[-1]),gs3simdatalist)
elif str(sys.argv[5])=="p":
    np.save("pmpy_eqrat_lamp{}_ang{}_tmax{}_dt{}".format(str(lam).split('.')[-1],str(int(sys.argv[2])),int(t_max),str(dt).split('.')[-1]),gs3simdatalist)
# For lambda of 1
elif str(sys.argv[5])=="i":
    np.save("pmpy_eqrat_lam{}_ang{}_tmax{}_dt{}".format(str(int(sys.argv[1])),str(int(sys.argv[2])),str(int(sys.argv[3])),str(dt).split('.')[-1]),gs3simdatalist)
